...
: type(type), id(id), min_rad(min_rad), max_rad(max_rad)
{
  ...
  if (type == DynamixelType::MX) {
    ...
    dxl_rps_ratio = MX_RPS_PROFILE * M_PI / 30;  //  RPM TO rad/s
...
Dynamixel::~Dynamixel()
{
  // std::cout << "[Dynamixel] Finishing Dynamixel id : " <<
...
bool Dynamixel::get_dynamixel_status(dynamixel::GroupSyncRead & SyncRead)
{
  ...
  present_position_raw = (int32_t)SyncRead.getData(id, address(PRESENT_POSITION), 4);
  present_velocity_raw = (int32_t)SyncRead.getData(id, address(PRESENT_VELOCITY), 4);
  present_position = to_rad(present_position_raw);
  present_velocity = present_velocity_raw * dxl_rps_ratio;
  ...
  // if (error_status_int != 0 && reboot_seq_ ==
  // DynamixelRebootSequence::STABLE) {
...
bool Dynamixel::set_position(double position)
{
  ...
  goal_position_raw = to_position(goal_position);
  return true;
}
...
bool Dynamixel::set_velocity(double velocity)
{
  ...
  profile_velocity_raw = static_cast<int32_t>(profile_velocity / dxl_rps_ratio);
  return true;
}
...
bool Dynamixel::set_acceleration(double acceleration)
{
  ...
  profile_acceleration_raw = static_cast<int32_t>(profile_acceleration / dxl_acc_ratio);
  return true;
}
    ...
    switch (reboot_seq_) {
      ...
      case DynamixelRebootSequence::REBOOT_START:
        ...
        if (dxl_reboot_result != COMM_SUCCESS) {
          ...
          // std::cout << "[Dynamixel] Reboot start success for ID " <<
          // static_cast<int>(id)
          //           << std::endl;
          reboot_seq_ = DynamixelRebootSequence::REBOOT_PING;
        }
      ...
      case DynamixelRebootSequence::REBOOT_PING:
        ...
        if (dxl_reboot_result != COMM_SUCCESS) {
          ...
          // std::cout << "[Dynamixel] Ping success for ID " <<
          // static_cast<int>(id) << std::endl;
          reboot_seq_ = DynamixelRebootSequence::INITIALIZE;
        }
        break;
      case DynamixelRebootSequence::INITIALIZE:
        if (!set_single_indirect_address(packet_handler, port_handler)) {
          ...
          // std::cout << "[Dynamixel] Initialize success for ID " <<
          // static_cast<int>(id)
          //           << std::endl;
          reboot_seq_ = DynamixelRebootSequence::SET_TORQUE;
        }
